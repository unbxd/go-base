...
// WithDatadogEvents sets custom datadog event emitter for
// Tolerance
func WithDatadogEvents(stats *datadog.Config) BreakerOption {
	return func(tp *Breaker) error {
		// create dd client
		cl, err := datadog.NewClient(stats)
		if err != nil {
			return errors.Wrap(err, "create client failed")
		}
		metric.Registry.Register(
			plugins.NewDatadogCollectorWithClient(cl),
		)
		return err
	}
}
